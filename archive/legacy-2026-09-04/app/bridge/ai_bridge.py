...
def _fixup_records(original_text, recs):
    """模型输出后处理: 1) 相对日期确定性覆盖; 2) 剥离模型可能回显的注入前缀。"""
    fixed_date = _resolve_relative_date(original_text)
    for r in recs:
...
def _parse(text, think=None):
    ...
    try:
        ...
        enriched = "（今天是%s）%s" % (today_cn, text)
        recs = bridge.bookkeep(enriched, think=think, model=cfg["model"],
    ...
    return _fixup_records(text, recs), None
